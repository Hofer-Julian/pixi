...
/// Warning shown when a prefer-local solve writes the lock file.
///
...
pub const PREFER_LOCAL_LOCK_FILE_WARNING: &str = "`pixi.lock` was written from a solve restricted to locally available packages, so it may pin older versions than the channels offer.\nRe-run `pixi update` without `--prefer-local` to refresh it.";

/// Warning shown when prefer-local mode cannot cover the whole workspace.
///
...
pub const PREFER_LOCAL_PYPI_WARNING: &str = "prefer-local mode restricts conda packages only. This workspace has PyPI dependencies, which may still require network access.";
