use pixi_build_types::{BinaryPackageSpec, PackageSpec, ProjectModel, Target, Targets};
...
pub fn project_model_uses_v3(model: &ProjectModel) -> bool {
    model
        .build_flags
        .as_ref()
...
fn targets_use_v3(targets: &Targets) -> bool {
    targets.default_target.as_ref().is_some_and(target_uses_v3)
        || targets
            .targets
            .as_ref()
            .is_some_and(|targets| targets.values().any(target_uses_v3))
}

fn target_uses_v3(target: &Target) -> bool {
    target
        .host_dependencies
        .as_ref()
...
fn package_spec_uses_v3(spec: &PackageSpec) -> bool {
    match spec {
        PackageSpec::Binary(binary) => binary_package_spec_uses_v3(binary),
        PackageSpec::Source(_) | PackageSpec::PinCompatible(_) => false,
    }
}

fn binary_package_spec_uses_v3(spec: &BinaryPackageSpec) -> bool {
    spec.extras
        .as_ref()
        .is_some_and(|extras| !extras.is_empty())
        || spec.flags.as_ref().is_some_and(|flags| !flags.is_empty())
        || spec.condition.is_some()
        || spec
            .track_features
            .as_ref()
            .is_some_and(|track_features| !track_features.is_empty())
