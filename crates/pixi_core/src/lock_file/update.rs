...
impl<'p> LockFileDerivedData<'p> {
    ...
    pub fn write_to_disk(&self) -> miette::Result<()> {
        // A prefer-local solve records the newest versions available on this
        ...
        if self.workspace.config().prefer_local() {
            tracing::warn!("{}", pixi_consts::consts::PREFER_LOCAL_LOCK_FILE_WARNING);
            ...
            {
                tracing::warn!("{}", pixi_consts::consts::PREFER_LOCAL_PYPI_WARNING);
            }
        }
