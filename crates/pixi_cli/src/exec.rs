...
use pixi_command_dispatcher::prefer_local::prefer_local_exclusions;
...
pub async fn create_exec_prefix(
    ...
    let environment_hash =
        EnvironmentHash::new(specs.clone(), channels, platform, config.prefer_local());
    ...
    // the prefer-local exclusions itself rather than inheriting them.
    let excluded_candidates = if config.prefer_local() {
        let package_cache =
            PackageCache::new(cache_dir.join(pixi_consts::consts::CONDA_PACKAGE_CACHE_DIR));
        let index = package_cache
            .index()
            .into_diagnostic()
            .context("failed to read the package cache")?;
        prefer_local_exclusions(
            &index,
